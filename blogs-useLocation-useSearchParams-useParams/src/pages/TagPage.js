...
const TagPage = () => {
  ...
  const fetchDataByTag = async (tag) => {
    const res = await fetch(`${baseUrl}?tag=${tag}`);
    const data = await res.json();
    setPosts(data.posts);
  };

  useEffect(() => {
    fetchDataByTag(params.tag);
  }, []);
  ...
  return (
    <div>
      ...
      <button
       ...
       <h2>Blogs On <span className="font-bold">#{params.tag}</span></h2>     
      </div>
      {
              ...
              return (
                <div key={index} className="flex-col my-3 gap-6">
                  <p className="text-lg font-bold">
                    <NavLink to={`/blogs/${post.id}`}>{post.title}</NavLink>
                  </p>
                  <p>
